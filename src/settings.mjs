...
export const defaultRules = Object.freeze([
  { id: 'investigate-before-claim', enabled: true, phase: 'before', input: 'latest-user-message', customInput: '',
  ...
  { id: 'evidence-after-answer', enabled: true, phase: 'after', input: 'tool-results', customInput: '',
...
const ruleSchema = z.object({
  id: z.string().required(), enabled: z.boolean().default(true), phase: z.union([z.const('before'), z.const('after')]).required(),
...
const normalize = value => {
  ...
  const rules = value.rules.map(rule => {
    ...
    if (!['before', 'after'].includes(rule.phase) || (rule.phase === 'before'
      ...
      || rule.customInput.length > 24000 || rule.questionScript.length > 16000 || question.length > 8000
    ...
    return { id: rule.id, enabled: rule.enabled, phase: rule.phase, input, customInput: rule.customInput,
