...
const chunks = text => async function* () { yield { type: 'text-delta', index: 0, text }; yield { type: 'finish', reason: 'stop' } }()
...
test('tool calls are rejected by the judgement boundary', async () => {
  const stream = async function* () { yield { type: 'tool-call-delta', index: 0, id: 'x', argumentsDelta: '{}' }; yield { type: 'finish', reason: 'stop' } }()
