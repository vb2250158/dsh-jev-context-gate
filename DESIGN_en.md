...
`tool-before` and `tool-after` subscribe to the host tool pipeline. Rules save an exact tool name or prefix, input source, question, threshold, and option actions. Before actions can return a host `deny`; after actions attach `additionalContexts` or use the installed Rabi tools to send and query a configured group. Stable delivery IDs identify retries, and Jev's own Rabi calls do not retrigger the same rule. A cancellable timer polls replies while the current process is alive; a group action cannot be enabled without its exact destination.
